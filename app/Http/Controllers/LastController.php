<?php
	...
	class LastController extends Controller
	{
		...
		public function index(Request $request) {
			...
			$this->breadcrumbs->addCrumb(__('locale.text_last_page'), route(session('route_url') . '_last_viewed'));
			$data['products'] = [];
			$data['total'] = 0;
			$page = 0;
			...
			$meta = [
				'name' => __('locale.text_last_page'),
				'meta_title' => __('locale.text_last_page'),
				'meta_description' => '',
				'meta_keywords' => ''
			];
			...
			$data['title'] = __('locale.text_last_page');
			
			$data['order'] = $order;
			$data['canonical'] = false;
			$data['class'] = 'catalog_last_viewed';
			$data['filters'] = [];
			$data['select_filters'] = [];
			...
			if (session('product_visited')) {
						...
						if (!$filters->isEmpty()) {
							...
							foreach ($filters as $filter_row => $filter) {
								if ($filter->type != 'slider') {
									$value_slug_last_viewed = $value_slug_param[$filter->slug];
									...
									$filter_values = $filter->select('fv.id', 'fv.slug', 'fvd.name')
										...
										->whereIn('fv.slug', $value_slug_last_viewed)
										->orderBy('fvd.name')
										->get();
									...
									if (isset($value_slug_last_viewed[0])) {
										$value_slug_last_viewed = explode('-', $value_slug_last_viewed[0]);
										
										if (!is_numeric($value_slug_last_viewed[0]) || (isset($value_slug_last_viewed[1]) && !is_numeric($value_slug_last_viewed[1])) || count($value_slug_last_viewed) > 2) {
											foreach ($value_slug_last_viewed as &$s) {
												$s = preg_replace('/[0-9]/i', '', $s);
											}
											
											$slug .= '/' . $filter->slug . '/' . implode('-', $value_slug_last_viewed);
											continue;
										}
										
										$min[$filter->id] = $value_slug_last_viewed[0];
										$max[$filter->id] = isset($value_slug_last_viewed[1]) ? $value_slug_last_viewed[1] : 0;
										
										if ($value_slug_last_viewed) {
				...
				$aliasPath = route(session('route_url') . '_last_viewed') . ($filter_ids ? '/' . ($price_range ? 'price/' . $price_start . '-' . $price_end . '/' : '') . implode('/', $params) : ($price_range ? '/price/' . $price_start . '-' . $price_end : ''));
				
				$path = route(session('route_url') . '_last_viewed') . ($this->lang != session('settings.default_language') ? '/' . $this->lang : '') . ($region ? '/' . $region . '/' : '') . $path;
				...
				$results->where('products.status', 1)
					->whereIn('products.id', (array)session('product_visited'))
				...
				if (isset($results) && $prev = $results->previousPageUrl()) {
					$header->setLinkData([
						[
							'href' => route(session('route_url') . '_last_viewed') . ($sort_url ? ('/' . 'sort-' . $sort . '/' . $order) : '') . str_replace(['?page=', '&page='], '/page/', str_replace(url()->current(), '', $prev)),
							'rel' => 'prev'
						]
					]);
				}
				...
				if (isset($results) && $last = $results->nextPageUrl()) {
					$data['next'] = route(session('route_url') . '_last_viewed') . ($sort_url ? ('/' . 'sort-' . $sort . '/' . $order) : '') . str_replace(['?page=', '&page='], '/page/', str_replace(url()->current(), '', $last));
				...
				$data['sorts'] = [
					[
						...
						'url' => route(session('route_url') . '_last_viewed') . $url . '/sort-price/asc' . ($page > 1 ? '/page/' . $page : ''),
						'active' => $sort == 'price' && $data['order'] == 'asc' ? 1 : 0
					],
					[
						'name' => __('locale.text_sort_price_desc'),
						'url' => route(session('route_url') . '_last_viewed') . $url . '/sort-price/desc' . ($page > 1 ? '/page/' . $page : ''),
						'active' => $sort == 'price' && $data['order'] == 'desc' ? 1 : 0
					],
					[
						'name' => __('locale.text_sort_name_asc'),
						'url' => route(session('route_url') . '_last_viewed') . $url . '/sort-name/asc' . ($page > 1 ? '/page/' . $page : ''),
					...
					[
						...
						'url' => route(session('route_url') . '_last_viewed') . $url . '/sort-popular/asc' . ($page > 1 ? '/page/' . $page : ''),
						'active' => $sort == 'popular' && $data['order'] == 'asc' ? 1 : 0
					],
					[
						'name' => __('locale.text_sort_popular_desc'),
						'url' => route(session('route_url') . '_last_viewed') . $url . '/sort-popular/desc' . ($page > 1 ? '/page/' . $page : ''),
						'active' => $sort == 'popular' && $data['order'] == 'desc' ? 1 : 0
					]
				];
				
				$data['price_range'] = $price_range;
				$data['new_url'] = route(session('route_url') . '_last_viewed');
			}
			...
			return render_view(view('pages.site.last', $data), $this->region);
		}
		...
		public function filter(Request $request) {
			...
			if ($request->filter || $request->filter_range) {
				...
				if (isset($request->filter_range['p'])) {
					...
					])->whereIn('products.id', (array)session('product_visited'))->get()->toArray();
			...
			if (isset($url)) {
				$url = route(session('route_url') . '_last_viewed') . (!empty($url) ? $url : '');
