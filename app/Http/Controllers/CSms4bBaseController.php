<?php
	...
	class CSms4bBaseController extends Controller {
...
",$arrHeader)."\r\n\r\n".$xml;
			...
			if ($response != -1)
			{
				switch ($funcname)
				{
					case "StartSession":
						...
						{
							session(["SMS_START_SESSION" => $this->sid]);
							$this->LastError = '';
							return true;
						}
						break;
		...
		public function getLogin()
		{
			...
			return isset($login['2']) ? $login['2'] : $this->login;
		}
	...
	*/
		...
		{
			return isset($this->arBalance["Addresses"]) ? $this->arBalance["Addresses"] : '';
		}
		...
		protected function UpdateSID()
		{
			...
			if (is_null(session('SMS_START_SESSION')))
			{
				$this->MakeSID();
			}
			else
			{
				...
				if(!$this->GetSOAP("AccountParams",array("SessionID" => session('SMS_START_SESSION'))))
				{
					$this->MakeSID();
				}
				else
				{
					$this->sid = session('SMS_START_SESSION');
				}
			}
		}
