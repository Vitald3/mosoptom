<?php
	...
	class SearchController extends Controller
	{
		...
		public function index(Request $request) {
			...
			$path = '';
			$search_url = route(session('route_url') . '_search');
			$header->setRobots('noindex, nofollow');
			$this->breadcrumbs->addCrumb(sprintf(__('locale.text_search_breadcrumb'), $search), route(session('route_url') . '_search'));
			$data['products'] = [];
			$data['total'] = 0;
			$page = 0;
			...
			$meta = [
				'name' => $search,
				'meta_title' => sprintf(__('locale.text_search_breadcrumb'), $search),
				'meta_description' => '',
				'meta_keywords' => ''
			];
			...
			$data['title'] = sprintf(__('locale.text_search_breadcrumb'), $search);
			
			$data['order'] = $order;
			$data['canonical'] = false;
			$data['class'] = 'catalog_search';
			$data['filters'] = [];
			$data['select_filters'] = [];
			...
			if ($request->search) {
				...
				$aliasPath = $search_url . ($filter_ids ? '/' . ($price_range ? 'price/' . $price_start . '-' . $price_end . '/' : '') . implode('/', $params) : ($price_range ? '/price/' . $price_start . '-' . $price_end : ''));
				
				$path = $search_url . ($this->lang != session('settings.default_language') ? '/' . $this->lang : '') . ($region ? '/' . $region . '/' : '') . $path;
			
				if ($aliasPath != $path) {
					abort(404);
				}
				
				$searchs = explode(' ', trim(preg_replace('/\s+/', ' ', urldecode($request->search))));
				
				$result = [];
				
				foreach ($searchs as $search) {
					$result[] = " ovd.name like '%" . $search . "%' or od.name like '%" . $search . "%'";
				}
				...
				$results->where('products.status', 1)
					...
					->where(function ($query) use ($searchs, $result) {
						foreach ($searchs as $search) {
							$query->where('pd.name', 'like', '%' . $search . '%')
								->Orwhere('products.model', 'like', '%' . $search . '%');
						}
						
						$query->OrwhereRaw("(select count(*) from product_attribute as pa where pa.product_id = products.id and pa.lang = ?) > 0 or
		    			(select count(*) from product_option as po left join product_option_values as pov on po.id = pov.product_option_id
			    		left join options as o on o.id = po.option_id left join option_description as od on od.option_id = o.id
				    	left join option_values as ov on ov.option_id = o.id left join option_value_description as ovd on ovd.option_value_id = ov.id
					    where po.product_id = products.id and o.status = '1' and ovd.lang = ? and od.lang = ? and " . \DB::raw($result) . ") > 0",
				...
				$results = $results->paginate(session('settings.limit_sait', 25), ['*'], 'page', $page);
				...
				if (isset($results) && $prev = $results->previousPageUrl()) {
					$header->setLinkData([
						[
							'href' => route(session('route_url') . '_search') . ($sort_url ? ('/' . 'sort-' . $sort . '/' . $order) : '') . str_replace(['?page=', '&page='], '/page/', str_replace(url()->current(), '', $prev)) . '?search=' . $search,
							'rel' => 'prev'
						]
					]);
				}
				...
				if (isset($results) && $last = $results->nextPageUrl()) {
					$data['next'] = route(session('route_url') . '_search') . ($sort_url ? ('/' . 'sort-' . $sort . '/' . $order) : '') . str_replace(['?page=', '&page='], '/page/', str_replace(url()->current(), '', $last)) . '?search=' . $search;
				...
				$data['sorts'] = [
					[
						...
						'url' => route(session('route_url') . '_search') . $url . '/sort-price/asc' . ($page > 1 ? '/page/' . $page : '') . '?search=' . $search,
						'active' => $sort == 'price' && $data['order'] == 'asc' ? 1 : 0
					],
					[
						'name' => __('locale.text_sort_price_desc'),
						'url' => route(session('route_url') . '_search') . $url . '/sort-price/desc' . ($page > 1 ? '/page/' . $page : '') . '?search=' . $search,
						'active' => $sort == 'price' && $data['order'] == 'desc' ? 1 : 0
					],
					[
						'name' => __('locale.text_sort_name_asc'),
						'url' => route(session('route_url') . '_search') . $url . '/sort-name/asc' . ($page > 1 ? '/page/' . $page : '') . '?search=' . $search,
					...
					[
						...
						'url' => route(session('route_url') . '_search') . $url . '/sort-popular/asc' . ($page > 1 ? '/page/' . $page : '') . '?search=' . $search,
						'active' => $sort == 'popular' && $data['order'] == 'asc' ? 1 : 0
					],
					[
						'name' => __('locale.text_sort_popular_desc'),
						'url' => route(session('route_url') . '_search') . $url . '/sort-popular/desc' . ($page > 1 ? '/page/' . $page : '') . '?search=' . $search,
						'active' => $sort == 'popular' && $data['order'] == 'desc' ? 1 : 0
					]
				];
				
				$data['price_range'] = $price_range;
				$data['search_url'] = route(session('route_url') . '_search', ['search' => $search]);
			}
			...
			return render_view(view('pages.site.search', $data), $this->region);
		}
		...
		public function search(Request $request)
		{
			...
			if ($request->search) {
				...
				$products = Products::with([
					...
					->where(function ($query) use ($searchs, $result) {
						foreach ($searchs as $search) {
							$query->where('pd.name', 'like', '%' . $search . '%')
								//->Orwhere('pd.description', 'like', '%' . $search . '%')
								->Orwhere('products.model', 'like', '%' . $search . '%');
						}
			    		...
			    		left join options as o on o.id = po.option_id left join option_description as od on od.option_id = o.id
				    	left join option_values as ov on ov.option_id = o.id left join option_value_description as ovd on ovd.option_value_id = ov.id
					    where po.product_id = products.id and o.status = '1' and ovd.lang = '?' and od.lang = '?' and ?) > 0",
							[$this->lang, $this->lang, $this->lang, $result]);
					})->limit(50)->get();
				
				$categories = Categories::join('category_description as cd', 'cd.category_id', '=', 'categories.id')
					->select('cd.name')
					->where('categories.status', 1)
					->where('cd.lang', $this->lang)
					->where(function ($query) use ($searchs, $result) {
						foreach ($searchs as $search) {
				...
				if (!$categories->isEmpty()) {
					foreach ($categories as $category) {
						$name = $this->setMarkForSearchValue($category->name, urldecode($request->search));
				...
				$data['count'] = sprintf(__('locale.search_count_product'), num_decline($products->count(), [__('locale.text_prod1'), __('locale.text_prod2'), __('locale.text_prod3')]));
				...
				if (!$products->isEmpty()) {
					foreach ($products as $product) {
						...
						if ((session('customer_id') || !$this->settings['price_logged']) && !is_null($product->product_special_one)) {
							$special = format_price($product->product_special_one->price, session('currency', []));
						} else {
							$special = false;
						}
						
						$name = $this->setMarkForSearchValue($product->name, urldecode($request->search));
						$model = $this->setMarkForSearchValue($product->model, urldecode($request->search));
						
						$data['products'][] = [
							'id' => $product->id,
							'name' => $name,
							'model' => __('locale.sku') . ' ' . $model,
							'url' => $product->getSlug(),
			...
			return view('pages.site.post_search', $data);
		}
		
		private function setMarkForSearchValue($text, $searchValue)
		{
			if (empty($searchValue)) return $text;
			
			$text = preg_replace('/'.$searchValue.'/iu', '<span class="search-highlight">$0</span>', $text);
		...
		public function filter(Request $request) {
			...
			if ($request->filter || $request->filter_range) {
				...
				if (isset($request->filter_range['p'])) {
					$price = (array)Products::distinct()->join('product_description as pd', 'pd.product_id', '=', 'products.id')
						->select('products.id', 'products.price')->with([
						'product_special_one:product_id,price',
			...
			if (isset($url)) {
				$url = route(session('route_url') . '_search') . (!empty($url) ? $url : '') . '?search=' . urlencode($request->search);
