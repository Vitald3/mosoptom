<?php
	...
	class BestsellerController extends Controller
	{
		...
		public function index(Request $request) {
			...
			$this->breadcrumbs->addCrumb(__('locale.text_bestseller_page'), route(session('route_url') . '_bestseller'));
			$data['products'] = [];
			$data['total'] = 0;
			$page = 0;
			...
			$meta = [
				'name' => __('locale.text_bestseller_page'),
				'meta_title' => __('locale.text_bestseller_page'),
				'meta_description' => '',
				'meta_keywords' => ''
			];
			...
			$data['title'] = __('locale.text_bestseller_page');
			
			$data['order'] = $order;
			$data['canonical'] = false;
			$data['class'] = 'catalog_bestseller';
			$data['filters'] = [];
			$data['select_filters'] = [];
						...
						if (!$filters->isEmpty()) {
							...
							foreach ($filters as $filter_row => $filter) {
								if ($filter->type != 'slider') {
									$value_slug_bestseller = $value_slug_param[$filter->slug];
									...
									$filter_values = $filter->select('fv.id', 'fv.slug', 'fvd.name')
										...
										->whereIn('fv.slug', $value_slug_bestseller)
										->orderBy('fvd.name')
										->get();
									...
									if (isset($value_slug_bestseller[0])) {
										$value_slug_bestseller = explode('-', $value_slug_bestseller[0]);
										
										if (!is_numeric($value_slug_bestseller[0]) || (isset($value_slug_bestseller[1]) && !is_numeric($value_slug_bestseller[1])) || count($value_slug_bestseller) > 2) {
											foreach ($value_slug_bestseller as &$s) {
												$s = preg_replace('/[0-9]/i', '', $s);
											}
											
											$slug .= '/' . $filter->slug . '/' . implode('-', $value_slug_bestseller);
											continue;
										}
										
										$min[$filter->id] = $value_slug_bestseller[0];
										$max[$filter->id] = isset($value_slug_bestseller[1]) ? $value_slug_bestseller[1] : 0;
										
										if ($value_slug_bestseller) {
				...
				$aliasPath = route(session('route_url') . '_bestseller') . ($filter_ids ? '/' . ($price_range ? 'price/' . $price_start . '-' . $price_end . '/' : '') . implode('/', $params) : ($price_range ? '/price/' . $price_start . '-' . $price_end : ''));
				
				$path = route(session('route_url') . '_bestseller') . ($this->lang != session('settings.default_language') ? '/' . $this->lang : '') . ($region ? '/' . $region . '/' : '') . $path;
				...
				$results = $results->orderBy('products.created_at', 'desc')->groupBy('products.id')->paginate(session('settings.limit_sait', 25), ['*'], 'page', $page);
				...
				if (isset($results) && $prev = $results->previousPageUrl()) {
					$header->setLinkData([
						[
							'href' => route(session('route_url') . '_bestseller') . ($sort_url ? ('/' . 'sort-' . $sort . '/' . $order) : '') . str_replace(['?page=', '&page='], '/page/', str_replace(url()->current(), '', $prev)),
							'rel' => 'prev'
						]
					]);
				}
				...
				if (isset($results) && $last = $results->nextPageUrl()) {
					$data['next'] = route(session('route_url') . '_bestseller') . ($sort_url ? ('/' . 'sort-' . $sort . '/' . $order) : '') . str_replace(['?page=', '&page='], '/page/', str_replace(url()->current(), '', $last));
				...
				$data['sorts'] = [
					[
						...
						'url' => route(session('route_url') . '_bestseller') . $url . '/sort-price/asc' . ($page > 1 ? '/page/' . $page : ''),
						'active' => $sort == 'price' && $data['order'] == 'asc' ? 1 : 0
					],
					[
						'name' => __('locale.text_sort_price_desc'),
						'url' => route(session('route_url') . '_bestseller') . $url . '/sort-price/desc' . ($page > 1 ? '/page/' . $page : ''),
						'active' => $sort == 'price' && $data['order'] == 'desc' ? 1 : 0
					],
					[
						'name' => __('locale.text_sort_name_asc'),
						'url' => route(session('route_url') . '_bestseller') . $url . '/sort-name/asc' . ($page > 1 ? '/page/' . $page : ''),
					...
					[
						...
						'url' => route(session('route_url') . '_bestseller') . $url . '/sort-popular/asc' . ($page > 1 ? '/page/' . $page : ''),
						'active' => $sort == 'popular' && $data['order'] == 'asc' ? 1 : 0
					],
					[
						'name' => __('locale.text_sort_popular_desc'),
						'url' => route(session('route_url') . '_bestseller') . $url . '/sort-popular/desc' . ($page > 1 ? '/page/' . $page : ''),
						'active' => $sort == 'popular' && $data['order'] == 'desc' ? 1 : 0
					]
				];
				
				$data['price_range'] = $price_range;
				$data['new_url'] = route(session('route_url') . '_bestseller');
			...
			return render_view(view('pages.site.bestseller', $data), $this->region);
		}
		...
		public function filter(Request $request) {
			...
			if ($request->filter || $request->filter_range) {
				...
				if (isset($request->filter_range['p'])) {
					...
					])->join('order_product as op', 'op.product_id', '=', 'products.id')->where('op.created_at', '>=', DB::raw('DATE_SUB(NOW(), INTERVAL 1 YEAR)'))->get()->toArray();
			...
			if (isset($url)) {
				$url = route(session('route_url') . '_bestseller') . (!empty($url) ? $url : '');
