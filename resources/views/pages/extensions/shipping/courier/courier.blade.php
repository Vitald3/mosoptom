...
@section('title','Доставка курьером')
                        ...
                        <div class="row">
                            <div class="col-12 col-sm-12">
                                <table class="table">
                                    ...
                                    <tbody id="add_tk">
									<?php $courier_row = 0; ?>
                                    @if(!empty($setting['courier']) && old('setting.courier', $setting['courier']))
                                        @foreach(old('setting.courier', $setting['courier']) as $courier)
                                            <tr id="tk_row{{ $courier_row }}">
                                                <td><input type="text" name="setting[courier][{{ $courier_row }}][name]" value="{{ $courier['name'] }}" placeholder="Название ТК" class="form-control" /></td>
                                                <td><input type="text" name="setting[courier][{{ $courier_row }}][cost]" value="{{ $courier['cost'] }}" placeholder="Стоимость доставки" class="form-control" /></td>
                                                <td><a href="#" class="btn btn-danger" onclick="$('#tk_row{{ $courier_row }}').remove();return false;">&times;</a></td>
                                            </tr>
											<?php $courier_row++; ?>
                                        @endforeach
                                    @endif
...
@section('page-scripts')
    <script>
        var tk_row = {{ $courier_row }};

        function addTk() {
            var html = '<tr id="tk_row' + tk_row + '">' +
                '  <td><input type="text" name="setting[courier][' + tk_row + '][name]" placeholder="Название" class="form-control" /></td>' +
                '  <td><input type="text" name="setting[courier][' + tk_row + '][cost]" placeholder="Стоимость доставки" class="form-control" /></td>' +
