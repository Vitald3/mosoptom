...
@section('content')
    <section class="users-list-wrapper">
        <div class="users-list-filter px-1">
            <form action="{{ asset('admin/customer_groups') }}" method="get">
                <div class="row bcustomer_group rounded py-2 mb-2" style="background: #fff">
                    ...
                    <div class="col-12 col-sm-6 col-lg-6 d-flex align-items-center" style="justify-content: space-between">
                        ...
                        <a href="{{ asset('admin/customer_groups') }}" style="margin-top:0;width: 48%" class="btn btn-primary btn-block glow users-list-clear mb-0">Очистить</a>
                    </div>
                </div>
            </form>
        </div>
            ...
            <div class="card">
                <div class="card-content">
                    <div class="card-body">
                        ...
                        @endif
                        <form action="{{ asset('admin/customer_group_delete') . $params }}" method="post" class="table-responsive">
                            @csrf
                            <div class="col-12 col-sm-12 d-flex align-items-center" style="justify-content: space-between;margin-bottom: 15px">
                                ...
                                @endrole
                                @role('create')
                                <a href="{{ asset('admin/customer_group_add') . $params }}" class="btn btn-primary btn-block glow users-list-clear mb-0 mt-0" style="width: 48%">Создать</a>
                                @endrole
                            </div>
                            <table id="users-list-datatable" class="table">
                                ...
                                <tr>
                                    ...
                                    @endrole
                                    <th>
                                        <a href="{{ $sort_name }}" class="{{ $sort == 'cgd.name' ? $order : '' }}">Название</a>
                                    </th>
                                    <th>
                                        <a href="{{ $sort_order }}" class="{{ $sort == 'customer_groups.sort_order' ? $order : '' }}">Порядок сортировки</a>
                                    </th>
                                    <th><a href="{{ $sort_status }}" class="{{ $sort == 'customer_groups.status' ? $order : '' }}">Статус</a></th>
                                ...
                                @if(!$customer_groups->isEmpty())
                                    @foreach($customer_groups as $customer_group)
                                        <tr>
                                            @role('delete')
                                            <td><input type="checkbox" name="selected[]" value="{{ $customer_group['id'] }}" /></td>
                                            @endrole
                                            <td>{{ $customer_group['name'] }}</td>
                                            <td>{{ $customer_group['sort_order'] }}</td>
                                            <td>{{ $customer_group['status'] == 1 ? 'Включено' : 'Выключено' }}</td>
                                            @role('edit|content_edit')
                                            <td><a href="{{asset('admin/customer_group/' . $customer_group['id']) . $params}}"><i class="bx bx-edit-alt"></i></a></td>
                                            @endrole
                                        </tr>
                                    @endforeach
                                @else
                            ...
                            {{ $customer_groups->appends($params_array)->links() }}
                        </form>
                    </div>
                </div>
            </div>
