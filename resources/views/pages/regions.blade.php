...
@section('content')
    <section class="users-list-wrapper">
        <div class="users-list-filter px-1">
            <form action="{{ asset('admin/regions') }}" method="get">
                <div class="row border rounded py-2 mb-2" style="background: #fff">
                    ...
                    <div class="col-12 col-sm-6 col-lg-6 d-flex align-items-center" style="justify-content: space-between">
                        ...
                        <a href="{{ asset('admin/regions') }}" style="margin-top:0;width: 48%" class="btn btn-primary btn-block glow users-list-clear mb-0">Очистить</a>
                    </div>
                </div>
            </form>
        </div>
            ...
            <div class="card">
                <div class="card-content">
                    <div class="card-body">
                        ...
                        @endif
                        <form action="{{ asset('admin/region_delete') . $params }}" method="post" class="table-responsive">
                            @csrf
                            <div class="col-12 col-sm-12 d-flex align-items-center" style="justify-content: space-between;margin-bottom: 15px">
                                ...
                                @endrole
                                @role('create')
                                <a href="{{ asset('admin/region_add') . $params }}" class="btn btn-primary btn-block glow users-list-clear mb-0 mt-0" style="width: 48%">Создать</a>
                                @endrole
                            </div>
                            <table id="users-list-datatable" class="table">
                                ...
                                @if(!$regions->isEmpty())
                                    @foreach($regions as $region)
                                        <tr>
                                            @role('delete')
                                            <td><input type="checkbox" name="selected[]" value="{{ $region['id'] }}" /></td>
                                            @endrole
                                            <td>{{ $region['name'] }}</td>
                                            <td>{{ $region['status'] == 1 ? 'Включено' : 'Выключено' }}</td>
                                            @role('edit|content_edit')
                                            <td><a href="{{asset('admin/region/' . $region['id']) . $params}}"><i class="bx bx-edit-alt"></i></a></td>
                                            @endrole
                                        </tr>
                                    @endforeach
                                @else
                            ...
                            {{ $regions->appends($params_array)->links() }}
                        </form>
                    </div>
                </div>
            </div>
