...
@section('title','Отзывы о товарах')
@section('content')
    <section class="users-list-wrapper">
        <div class="users-list-filter px-1">
            <form action="{{ asset('admin/reviews') }}" method="get">
                <div class="row border rounded py-2 mb-2" style="background: #fff">
                    ...
                    <div class="col-12 col-sm-6 col-lg-3">
                        <label for="users-list-role">Автор</label>
                        <fieldset class="form-group">
                            <input type="text" name="name" placeholder="Автор" value="{{ $name }}" class="form-control" />
                        </fieldset>
                    </div>
                    <div class="col-12 col-sm-6 col-lg-6 d-flex align-items-center" style="justify-content: space-between">
                        ...
                        <a href="{{ asset('admin/reviews') }}" style="margin-top:0;width: 48%" class="btn btn-primary btn-block glow users-list-clear mb-0">Очистить</a>
                    </div>
                </div>
            </form>
        </div>
            ...
            <div class="card">
                <div class="card-content">
                    <div class="card-body">
                        ...
                        @endif
                        <form action="{{ asset('admin/review_delete') . $params }}" method="post" class="table-responsive">
                            @csrf
                            <div class="col-12 col-sm-12 d-flex align-items-center" style="justify-content: space-between;margin-bottom: 15px">
                                ...
                                @endrole
                                @role('create')
                                <a href="{{ asset('admin/review_add') . $params }}" class="btn btn-primary btn-block glow users-list-clear mb-0 mt-0" style="width: 48%">Создать</a>
                                @endrole
                            </div>
                            <table id="users-list-datatable" class="table">
                                ...
                                <tr>
                                    ...
                                    @endrole
                                    <th>
                                        <a href="{{ $sort_name }}" class="{{ $sort == 'author' ? $order : '' }}">Автор</a>
                                    </th>
                                    <th>Товар</th>
                                    <th>
                                        <a href="{{ $sort_rating }}" class="{{ $sort == 'rating' ? $order : '' }}">Рейтинг</a>
                                    </th>
                                ...
                                @if(!$reviews->isEmpty())
                                    @foreach($reviews as $review)
                                        <tr>
                                            ...
                                            @endrole
                                            <td>{{ $review['author'] }}</td>
                                            <td>@if(!is_null($review->product))<a href="{{ url('admin/product/' . $review->product['product_id']) }}">{{ $review->product['name'] }}</a>@endif</td>
                                            <td>{{ $review['rating'] }}</td>
                                            <td>{{ $review['status'] == 1 ? 'Включено' : 'Выключено' }}</td>
                                            @role('edit|content_edit')
                                            <td><a href="{{asset('admin/review/' . $review['id']) . $params}}"><i class="bx bx-edit-alt"></i></a></td>
                                            @endrole
                                        </tr>
                                    @endforeach
                                @else
                            ...
                            {{ $reviews->appends($params_array)->links() }}
                        </form>
                    </div>
                </div>
            </div>
