                                        ...
                                        <div class="form-group">
                                            <select name="layout_id" class="form-control">
                                                ...
                                                @foreach($layouts as $layout)
                                                    <option value="{{ $layout->id }}"{{ $layout->id == old('layout_id', $layout_id) || (!old('layout_id', $layout_id) && $layout->route == 'page_category') ? ' selected' : '' }}>{{ $layout->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
        ...
        $(document).ready(function () {
            $('[name="parent"]').autocomplete({
                source: function(request, response) {
                    $.ajax( {
                        url: '{{ asset('admin/page_category_autocomplete') }}?id={{ $id ? $id : 0 }}',
                        dataType: "json",
                        data: {
                            term: request.term
                        },
