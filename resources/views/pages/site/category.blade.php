...
@section('content')
    <section class="container" id="content">
        <div class="flex wrap flex-start relative">
        ...
        @if($aside)
            <div class="popular_folder">
                <div class="elements row4 flex-2 flex-start wrap">
                    @foreach($aside as $category)
                        <div class="cats">
                            <a href="{{ $category['url'] }}" class="element relative">
                                <div class="element_name">{{ $category['name'] }}</div>
                                @if($category['image_cat'])
                                    <img src="{{ $category['image_cat'] }}" alt="{{ $category['name'] }}" />
                                @endif
                            </a>
                        </div>
                    @endforeach
        ...
        <div id="products" class="col-12">
            @if($top)
                <div class="flex-2 top_filters flex-start wrap">
                    @foreach($top as $filter)
                ...
                @if(!empty($products))
                    @if($next)
                        <a href="{{ $next }}" class="load_more">
                            <span>{{ sprintf(__('locale.text_load_more_catalog'), num_decline($more, [__('locale.text_prod1'), __('locale.text_prod2'), __('locale.text_prod3')])) }}</span>
                            <div class="fill"></div>
                        </a>
                    @endif
                    {!! $products->links() !!}
...
@section('page-scripts')
            ...
            function reset_filter() {
                $.ajax({
                    ...
                    success: function(html) {
                        ...
                        selectize('.selectize');
                        wishlist.getList();
                        window.history.pushState('', '', '{{ $category_url }}');
    ...
    <script>
        $(document).ready(function(){
            ...
            if ($('.content_bottom .forms').length && $('.products .product').length >= length) {
                var forms = $('.forms').detach();
                $('.products .product:nth-last-child(' + length + ')').after(forms);
            }
        });
    ...
    @if(!empty($products))
		<?php $count = $products->count(); $key = 1; ?>
        <script type="application/ld+json">
{
  ...
  "itemListElement":[
    @foreach($products as $product){
      ...
      "position":{{ $key }},
      "url":"{{ $product->getSlug() }}"
    }<?php echo $key < $count ? ',' : ''; $key++; ?>@endforeach
            ]}
</script>
    @endif
@endsection